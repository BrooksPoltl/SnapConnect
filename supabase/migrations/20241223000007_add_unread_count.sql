-- Migration: Add Unread Message Count
-- Description: Updates get_user_conversations to include unread message count
...
BEGIN
    ...
    unread_counts AS (
        -- Count unread messages for each chat (messages sent by others that haven't been viewed)
        SELECT 
            m.chat_id,
            COUNT(*) as unread_count
        FROM public.messages m
        WHERE m.chat_id IN (SELECT ec.chat_id FROM existing_chats ec WHERE ec.chat_id IS NOT NULL)
        AND m.sender_id != auth.uid()  -- Messages from others
        AND m.viewed_at IS NULL        -- Not yet viewed
    ...
    LEFT JOIN unread_counts uc ON uc.chat_id = ec.chat_id
